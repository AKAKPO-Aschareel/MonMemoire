...
%Rayleigh Parameters
sampleRate = 500000; %2.048e6    % Sample rate of 2.048  MHz
maxDopplerShift  = 0;      % Maximum Doppler shift of diffuse components (Hz)
delayVector1 = [0.0 0.1 0.2 0.3 0.4 5.0]; % Discrete delays of six-path channel (s)
delayVector = delayVector1.*10^-6; % Discrete delays of four-path channel (s)
gainVector  = [ 0 -4 -8 -12 -16 -20];  % Average path gains (dB)
KFactor=10;
specDopplerShift=0;
...
p=96 ; % depth of interleaving
 ...
 sym_interleaved = matintrlv(modSig,p,n);
...
ray_Out=ricChan(Tx_Frame_Final.');
...
semilogy (SNR,TEB,'b-o','MarkerSize',5,'MarkerFacecolor','b','linewidth',2);
grid on;
hold on;
xlabel('SNR(dB)');
ylabel('BER');
